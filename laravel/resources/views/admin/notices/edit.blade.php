            ...
            <div class="card-body">
                    ...
                    <select class="form-control @error('career_id') is-invalid @enderror" 
                        ...
                        @foreach($careers as $career)
                            <option value="{{ $career->id }}" 
                                {{ old('career_id', $notice->careerId) == $career->id ? 'selected' : '' }}>
                                {{ $career->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control @error('description') is-invalid @enderror" 
                              id="description" 
                              name="description" 
                              rows="3">{{ old('description', $notice->description) }}</textarea>
                    @error('description')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="date" 
                           ...
                           id="exam_date" 
                           name="exam_date" 
                           value="{{ old('exam_date', $notice->examDate ? \Carbon\Carbon::parse($notice->examDate)->format('Y-m-d') : '') }}">
                    @error('exam_date')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    <div class="custom-control custom-switch">
                        <input type="checkbox" 
                               ...
                               name="active" 
                               value="1" 
                               {{ old('active', $notice->active) ? 'checked' : '' }}>
